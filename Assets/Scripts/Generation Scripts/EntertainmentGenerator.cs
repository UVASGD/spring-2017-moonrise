...
public class EntertainmentGenerator : mapGenerator
{
    ...
    {
        int[] loc = new int[] { 15, cent - 3 }; //Starting point is 15 tiles up, 3 tiles to the left of center

        while (loc[0] < 60) //Only generate offshoot roads until 60 tiles up
        {
            Builder(new int[,] { { loc[0], loc[1] - 20 }, { loc[0] + 3, loc[1] } }, 'g', 0); //Build 3 tile wide offshoot from main road

            for (int i = loc[1]-20; i < loc[1]-5; i += Random.Range(3, 8)) //Generate 2 tile wide road going up/down from 3 tile wide offshoot from main road
            {
